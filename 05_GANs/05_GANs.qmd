...
format: 
  revealjs:
    theme: "white" #["theme/q-theme.scss"]
    slide-number: c/t
...
## Rotationally Invariant VAEs Forward 1

- (A) In systems with continuous translational symmetry, the initial building blocks can be built using convolution with sliding window and subsequent dimensionality reduction using linear or nonlinear methods. Note that the same principle is used in CNNs. 
- (B) In systems with known atomic positions (or other physically defined reference points), the analysis can be performed on the basis of either the local atomic neighborhood of each atom or 
- (C) analysis of the connectivity graph between point objects. 

![Defining the building blocks for machine learning based on extant discrete and continuous symmetries in the system.](../img2/abd5084-f4.jpeg){ width=80% background="white"}

## Rotationally Invariant VAEs Forward 2

- image is reduced to the coordinates of a small number of features, and remaining information is discarded. 
- Last, local descriptors can be built on the basis of (D) continuous neighborhoods of individual atomic units (or other reference point);
- key element in dimensionality reduction then becomes the presence of (E) general rotational symmetry accounting for multiple potentially identical units having different orientations in the image plane.
